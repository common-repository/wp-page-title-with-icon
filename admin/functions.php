...
function wpptwi_insert_color_picker( $hook ) {
    ...
    if( is_admin() ) { 
		...
		wp_enqueue_script('icon-picker', WPPTWI_PLUGIN_URL.'/js/fontawesome-iconpicker.js', array( 'jquery' ));               
        // Include custom jQuery file with WordPress Color Picker dependency
        wp_enqueue_script( 'wpptwi-color-picker', WPPTWI_PLUGIN_URL.'/js/custom.js', array( 'jquery', 'wp-color-picker' ), false, true ); 
